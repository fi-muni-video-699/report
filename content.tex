...
solves task 1. In Section~\ref{sec:experimental-setup}, I will evaluate the
baseline system.
...
\begin{description}
  \item[Second-tier classifiers]
    ...
    Given a set of training recordings, a set of training observations is
    produced from all random vectors between a projection screen and the
    lecture slide pages from a single recording. Each training observation is
    assigned a \term{class} based on whether the projection screen matches any
    lecture slide page and whether the projection screen and the lecture slide
    pages correspond to the same recording.
...
to evaluate the system described in Section~\ref{sec:system-description}.
