...
similarity to a screen and, if there exists a matching page, it will be the
top-ranking one. To compare two systems, we may estimate the expected value of
the \term{rank} of a matching page produced by the two systems across one or
...
for the positive (when a screen matches) and the negative cases, then a
classifier can be trained on one or several training datasets using the
measures produced by a task 1a system as features.

If we only require that our system produce binary decisions, then we can
estimate the expected value of the \term{error rate} (the ratio between wrong
to right decisions) to compare two systems. If we also require that our system
produces a probability estimate, then we can estimate the expected value of a
„softer“ measure, such as the \term{logarithmic loss} (the inverse of the
log-likelihood~\cite[sec~2.6.3]{friedman2001elements}) which favors classifiers
that are hesitant about their wrong decisions and confident about their right
decisions.
