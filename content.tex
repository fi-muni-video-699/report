...
\begin{description}
  ...
  \item[vgg256] two feature vectors obtained by feeding the entire
    frame into two 256dimensional \term{\abbr{VGG} convolutional neural
...
Prior to solving task 1, the video frame images and lecture slide page images
are preprocessed in the \texttt{preprocessing.Image.get\_image} method. I will
now describe the individual steps in order. Examples are shown in figures~%
...
\begin{description}
  ...
  \begin{figure}
    \showtwoimages{fig/preprocessing}%
      {J}{J_}%
      {The morphological closing $\gamma(Y'_2)$}%
      {The morphological closing $\gamma(Y'_3)$}\par
    \showthreeimages{fig/preprocessing}%
    ...
    \caption{Binarozation of images $I_1,I_2$, and $I_3$.}
  ...
  \item[Uneven illumination removal]
    ...
    applying a \term{morphological closing} $\gamma$ with a rectangular
    \term{structural element} of size $\theta_1$, where $\theta_1$ is a
    parameter, and with the assumption that fall pixels beyond the image
