...
screen, an \term{inverse perspective transform} of the recording will be
necessary to \term{crop out} the region corresponding to a projection screen
and make it rectangular. Given an incorrectly positioned camera, or a camera
with \term{telephoto lens}, portions of the projection screen will fall
\term{beyond the bounds} of the captured scene.
...
\begin{description}
  ...
  \item[vgg256] two feature vectors obtained by feeding the entire
    ...
    \note{\url{http://www.image-net.org/}} image classification dataset and the
    second trained on the ImageNet and
    Places2\note{\url{http://places2.csail.mit.edu/}} datasets -- and taking
...
\begin{description}
  ...
  \begin{figure}
    ...
    \showthreeimages{fig/examples/no-keyrefs}%
      ...
      {The image file \texttt{frame004000.png}}%
      {A cropped-out \texttt{frame004000.png} (L)}%
      {A cropped-out \texttt{frame004000.png} (R)}
  ...
  \item[No matching pages] In 93 out of the total 683 screen elements, the
    ...
    that a system solving task 1b should be able to detect. An example is shown
