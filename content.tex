...
If we only require a system produce binary decisions, then we can estimate the
expected value of the \term{error rate} (the ratio between wrong to right
decisions) to compare two systems. If we also require that a system produce a
probability estimate, then we can estimate the expected value of a „softer“
measure, such as the \term{logarithmic loss} (the inverse of the
