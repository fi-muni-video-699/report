...
I will give the evaluation results.
  ...
  \item[similarity] the type of relation between this lit projection screen and
    ...
    \begin{description}
      ...
      \item[incremental] a single \term{logical} lecture slide page is split
        ...
        same logical page ($s\mathop{\hat\sim}p$).
    \end{description}
\end{description}
...
and only if \[s\sim p\lor\big(\nexists p'(s\sim p')\land s\mathop{\hat\sim}p\big).\]
...
dataset is used to train the models, the validation dataset is to select the
best-performing model, and the test dataset is used to estimate the performance
...
advantage of placing \term{confidence intervals} on the evaluation measures and the
disadvantage of increasing the time complexity of the evaluation by a
...
cross-validation was performed, i.e. no confidence intervals are given for the
...
by taking the percentiles from the ranks observed at the individual
...
by making a normality assumption about the binomial misclassification loss
and by taking the percentiles of the normal distribution.
